...
class io_broomerang extends IO {
    constructor(b) {
        super(b);
        this.r = 0; 
        this.b = b;
    ...
    think(input) {
        ...
        let t = 1; //1 - Math.sin(2 * Math.PI * this.b.range / this.r) || 1;
        if (!this.turnover) {
            if (this.r && this.b.range < this.r * 0.1) { this.turnover = true; }
            return {
                goal: this.myGoal,
                power: t,
            };
...
class io_scissors extends IO {
    ...
    buildList(range) {
        ...
        let out = entities.map(e => {
            ...
            if (e.master.master.team !== this.body.master.master.team) {
            if (e.master.master.team !== -101) {
            if (e.type === 'tank' || e.type === 'crasher' || e.type !== 'paper' || e.type === 'scissors' || e.type !== 'spec' || (!this.body.aiSettings.shapefriend && e.type === 'food')) {
            if (Math.abs(e.x - m.x) < range && Math.abs(e.y - m.y) < range) {
            if (!this.body.aiSettings.blind || (Math.abs(e.x - mm.x) < range && Math.abs(e.y - mm.y) < range)) return e;
            } } } } } }
        }).filter((e) => { return e; });        
...
class io_targetOwnType extends IO {
    constructor(body) {
        super(body);
        this.targetLock = undefined;
        this.tick = ran.irandom(30);
        ...
        this.avoid = nearest( 
            ...
            function(test, sqrdst) { 
                return (
                    ...
                    (test.type === 'bullet' || test.type === 'shockwave' || test.type === 'drone' || test.type === 'swarm' || test.type === 'trap' || test.type === 'block') &&
                    sqrdst < range
                ); }
        );
        // Aim at that target
...
class io_steroidspin extends IO {
    constructor(b) {
        super(b);
        this.a = 0;
    }    
...
class io_fleeAtLowHealth extends IO {
    ...
    think(input) {
        if (input.fire && input.target != null && this.body.health.amount < this.body.health.max * 0.25) {
            return {
                goal: {
...
class io_loseTarget extends IO {
    constructor(b) {
        super(b);
        this.boredom = util.clamp(ran.gauss(0.7, 0.15), 0.1, 0.9);
    }    
    think(input) {
        if (input.fire && input.target != null && this.body.health.amount < this.body.health.max * this.boredom) {
            return {
                goal: null,
                target: null,
            };
...
class io_whatever extends IO {
    ...
    think(input) {
        if (input.fire && input.target != null && this.body.health.amount < this.body.health.max * this.fear) {
            return {
                goal: {
                    x: input.target.x + this.body.y,
                    y: input.target.y + this.body.x,
                },
            };
        }
    }
}
class io_spinAtLowHealth extends IO {
    constructor(b) {
        super(b);
        this.a = 0;
        this.fear = util.clamp(ran.gauss(0.7, 0.15), 0.1, 0.9);
...
class io_class5 extends IO { 
    constructor(b) {
        super(b);
        this.doingthestuff = false
        
    }     
    think(input) {
        if (input.target != null && this.doingthestuff == false) {
          this.body.velocity.x = 0
          this.body.velocity.y = 0
          this.doingthestuff = true
          let tx = input.target.x
          let ty = input.target.y
          let angle = ran.randomAngle()
          let speed = ran.randomRange(400, 600)
          let xx = Math.cos(angle)*speed
          let xy = Math.sin(angle)*speed
          let newpos = { x: this.body.x + (tx+xx), y: this.body.y + (ty+xy) }
          let o = new Entity(newpos)
          o.define(Class.warnflash5)
          o.color = 5
          this.body.x = newpos.x
          this.body.y = newpos.y
          this.body.speed = speed/30
          this.body.acceleration = speed/30
          setTimeout(()=> {
            this.doingthestuff = false
            this.body.speed = 0
            ...
            this.body.velocity.x = 0
            this.body.velocity.y = 0
            this.body.x += ran.randomRange(2000, 10000)
            this.body.y += ran.randomRange(2000, 10000)
            this.target = null
          }, 1000)
...
reset() {
    ...
    update() {
        ...
        this.rld = Math.pow(0.7, attrib[skcnv.rld]);
        this.pen = apply(1.15, attrib[skcnv.pen]);
        this.str = apply(1.9, attrib[skcnv.str]);
        this.dam = apply(1.8, attrib[skcnv.dam]);
    ...
    get levelScore() {
        return Math.ceil(1.58647 * Math.pow(this.level + 1, 1.58647) - 2 * this.level + 1);
    }
    get progress() {
...
class Gun {
    constructor(body, info) {
        ...
        if (info.PROPERTIES != null && info.PROPERTIES.TYPE != null) {
            ...
            this.speedDebuff = (info.PROPERTIES.SPEED_DEBUFF == null) ?
                false : info.PROPERTIES.SPEED_DEBUFF;
            this.recoilDebuff = (info.PROPERTIES.RECOIL_DEBUFF == null) ?
                false : info.PROPERTIES.RECOIL_DEBUFF;
            this.abilityFire = (info.PROPERTIES.ABILITY_FIRE == null) ?
                false : info.PROPERTIES.ABILITY_FIRE;
            this.fovIncrease = (info.PROPERTIES.FOV_INCREASE == null) ?
                false : info.PROPERTIES.FOV_INCREASE;
            this.fireOnRelease = (info.PROPERTIES.FIRE_ON_RELEASE == null) ?
                false : info.PROPERTIES.FIRE_ON_RELEASE;
        ...
        if (this.canShoot) {
            ...
            this.specialcycle = !this.waitToCycle - this.delay;
    ...
    recoil() {
        if (this.motion || this.position) {
            // Simulate recoil
            this.motion -= 0.999999999 * this.position / roomSpeed; 
            ...
            if (this.motion > 0) {
                this.motion *= 0.0000000001;
            }
        }   
        if (this.canShoot && !this.body.settings.hasNoRecoil) {
            // Apply recoil to motion
            if (this.motion > 0) {
                let recoilForce = -this.position * this.trueRecoil * 0.2 / roomSpeed;
    ...
    live() {
        // Do 
        this.recoil();
        let alreadyDidThis2 = false;
      let is_shooting = true;
        // Dummies ignore this                                                                              erhjfdgshajfgshjhfuierbgj
        if (this.canShoot) {
            if (this.speedDebuff == true) {
              this.master.fov = 2000 - (this.children.length * 90)
            ...
            if (shootPermission || !this.waitToCycle) {
                ...
                if (this.specialcycle < 1) {
                    this.specialcycle += 1 / this.settings.reload / roomSpeed / ((this.calculator == 'necro' || this.calculator == 'fixed reload') ? 1 : sk.rld);
                }
            } 
            // Firing routines
          
          var this_ = this
            function idkwhattonamethislol() {
              console.log('starting function')
              console.log(is_shooting)
              let alreadyDidThis = false;
              shootPermission = false;
              let time = 1;   
              if (time <= 5000 && is_shooting == true) {   
              console.log("doing the thing")
              time += 1; }
                else if (is_shooting == false) {
                  console.log('isnt shooting')
                }
              if (alreadyDidThis == false && is_shooting == false && time > 1) {
              console.log('other conditions met')
              alreadyDidThis = true;
              console.log(time)   
              this_.recordedTime = time; 
              time = 0;
              shootPermission = true;  
                    if (shootPermission) {
                    console.log('firing')
                    let gx = 
                        this_.offset * Math.cos(this_.direction + this_.angle + this_.body.facing) +
                        (1.5 * this_.length - this_.width * this_.settings.size / 2) * Math.cos(this_.angle + this_.body.facing);
                    let gy =  
                        this_.offset * Math.sin(this_.direction + this_.angle + this_.body.facing) +
                        (1.5 * this_.length - this_.width * this_.settings.size / 2) * Math.sin(this_.angle + this_.body.facing); 
                        this_.fire(gx, gy, sk);   
                        this_.cycle -= 1; 
                        console.log('fired')
                    }
            ...
            //console.log("abilityFire: " + this.abilityFire)
           // console.log("thisthing (before fire): " + this.master.thisthing)
            if (shootPermission && (this.autofire || ((this.altFire) ? this.body.control.alt : this.body.control.fire) || (this.master.thisthing == true && this.abilityFire) && !this.fireOnRelease)) {
                if (this.cycle >= 1) {
              ...
              if ((this.master.thisthing == true && this.abilityFire) && this.specialcycle >= 1) {
                    // Find the end of the gun barrel
                    let gx = 
                        this.offset * Math.cos(this.direction + this.angle + this.body.facing) +
                        (1.5 * this.length - this.width * this.settings.size / 2) * Math.cos(this.angle + this.body.facing);
                    let gy = 
                        ...
                        (1.5 * this.length - this.width * this.settings.size / 2) * Math.sin(this.angle + this.body.facing); 
                    // Shoot, multiple times in a tick if needed
                    while (shootPermission && this.specialcycle >= 1) {
                        this.fire(gx, gy, sk);   
                        if (this.fovIncrease) { 
                          this.master.fov += (this.master.fov * 0.5)
                          this.master.thisthing = false;
                          setTimeout(()=> {
                            this.master.fov -= (this.master.fov * 0.3333333333)
                          }, 2500)
                          } 
                        // Figure out if we may still shoot
                        shootPermission = (this.countsOwnKids) ?
                ...
                this.cycle = !this.waitToCycle - this.delay;
            } else if (this.specialcycle > !this.waitToCycle - this.delay) {
                this.specialcycle = !this.waitToCycle - this.delay;
            }/*else if (this.fireOnRelease && alreadyDidThis2 == false && (this.altFire) ? this.body.control.alt : this.body.control.alt) {
                if (this.cycle >= 1) {
                console.log('conditions met')
           //   idkwhattonamethislol()
                console.log('function ran')
                alreadyDidThis2 = true;
            } else {
                alreadyDidThis2 = true;}
              } */
            }
...
var bringToLife = (() => {
    let remapTarget = (i, ref, self) => {
        if (i.target == null || (!i.main && !i.alt && i.ability)) return undefined;
        return {
    ...
    return my => {
        ...
        if (my.invisibluh[1]) {
          my.alpha = Math.max(0.01, my.alpha - my.invisibluh[1]);     
        }
...
class Entity {
    ...
    define(set) {
        ...
        if (set.CONTINUALLY_INCREASE_SIZE != null) { 
            this.settings.continuallyIncreaseSize = set.CONTINUALLY_INCREASE_SIZE; 
        }
        if (set.THOSE_WHO_DOZE != null) { 
            this.settings.thoseWhoDoze = set.THOSE_WHO_DOZE; 
        }
        ...
        if (set.INVISIBLUH != null) { 
            this.invisibluh = set.INVISIBLUH;
        }
        if (set.DANGER != null) { 
        ...
        if (set.CHILDREN_AMOUNT != null) { 
            this.childrenamount = set.CHILDREN_AMOUNT; 
        }
        if (set.FOOD != null) {
    ...
    refreshBodyAttributes() {
        ...
        this.childrenamount = this.children.length;  
          }    
   ...
   camera(tur = false) {
        return {
            ...
            layer: (this.bond != null) ? this.bound.layer : 
                    ...
                    (this.type === 'wall') ? 11 : 
                    (this.type === 'food') ? 10 : 
                    (this.type === 'indicator') ? 7 :
                    (this.type === 'miniboss') ? 6 :
                    (this.type === 'tank') ? 5 :
                    (this.type === 'shockwave') ? 4 :
                    (this.type === 'bullet') ? 3 :
                    (this.type === 'trap') ? 2 :
                    (this.type === 'crasher') ? 1 :
                    0,
    ...
    upgrade(number) {
        if (number < this.upgrades.length && this.skill.level >= this.upgrades[number].level) {     
            ...
            setTimeout(() => {  
            if (this.label == 'Eight-Fourty') {
              this.sendMessage('Altfire: use your shotgun cannon.');
            } 
            }, 1010);
            if (this.label == 'SmasherrrrrrrrrrRRRRRRRRRRRRRRRRRrrrr') {
              this.sendMessage('This tank has garbage acceleration but if you get poked by a Poker (or just wait a long time) you can pick up a shit ton of speed');
            }
            setTimeout(() => {
            if (this.label == 'Diversionist') {
              this.sendMessage("Altfire: deploy two bullets that will temporarily distract enemies.");
            }
            }, 1010);
            if (this.label == 'gorg') {
              this.sendMessage("GORG");
            }
            setTimeout(() => {
            if (this.label == 'mapwide fov') {
              this.sendMessage("WARNING: the game may break while you are using this spectator since mapwide FoV is not an intended feature for players.");
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == 'stippleTwo') {
              this.sendMessage("Altfire: deploys a trap generator, maximum of 4 at once.");
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == "'Thornrunner'") {
              this.sendMessage("Altfire: heavily increases your speed.");
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == "Falcon") {
              this.sendMessage("Ability [F]: Increase your FoV by 50% for 2.5 seconds. Cannot be used while shooting.")
              this.sendMessage("Altfire: use your front gun.");
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == 'ylovrgenedk ushaernzaum') {
              this.sendMessage("Altfire: begins charging up your thruster cannons.");
            }     
            }, 1010);
            setTimeout(() => {
            if (this.label == 'Bomber') {
              this.sendMessage("Altfire: place down a bomb.");
            }     
            }, 1010);
            setTimeout(() => {
            if (this.label == 'Surfer') {
              this.sendMessage("Altfire: Use your front-facing swarm spawners.");
            }     
            }, 1010);
            setTimeout(() => {
            if (this.label == '^V') {
              this.sendMessage("Altfire: cancel the recoil/boost from thruster cannons.");
            }     
            }, 1010);
            setTimeout(() => {
            if (this.label == 'Repeller') {
              this.sendMessage("Altfire: Send out a shockwave that can hit small or light enemies.");
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == 'U-Spawn') {
              this.sendMessage("Altfire: Spawn drones. Having more drones will result in a larger FoV debuff.");
               setTimeout(() => {
              this.sendMessage("The drones will explode if too many are spawned.");
               }, 2000)
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == 'sevruqunjhsc psgnholkp') {
              this.sendMessage("Altfire: Launch yourself in the opposite direction as you are facing.");
            }
            }, 1010);
            setTimeout(() => {
            if (this.label == 'Intuivujuv Klekjlkov') {
              this.sendMessage("This tank currently is not functional. It is intended to have a certain feature which i am not finished implementing yet.");
              this.upgrades = []
              this.define(Class.tri);
            }
            }, 1010);
            
           /*   this.sendMessage("Altfire: hold and release to deploy a bullet that will explode shortly after firing.");
              setTimeout(()=>{
                this.sendMessage("Holding altfire for longer will increase the power of the explosion, in exchange for a longer cooldown.");
              }, 2010)
            }
            }, 1010); */
            if (this.label == 'ELITA IS CRINGE') {
              this.sendMessage("how the fuck did you manage to upgrade to this");
            }
            if (this.label == 'Risingson') {
              sockets.broadcast("I seen you go down to a cold mirror"); setTimeout(() => {  sockets.broadcast("It was never clearer in my era so"); }, 2000); setTimeout(() => {  sockets.broadcast("You lick a shine upon your forehead or"); }, 4000); setTimeout(() => {  sockets.broadcast("Check it by the signs in the corridor"); }, 6000); setTimeout(() => {  sockets.broadcast("You light my ways through the club maze"); }, 8000); setTimeout(() => {  sockets.broadcast("We would struggle through the dub daze"); }, 10000); setTimeout(() => {  sockets.broadcast("I see myself in there upon my lover"); }, 15000); setTimeout(() => {  sockets.broadcast("It's how you go down to the men's room sink"); }, 17000); setTimeout(() => {  sockets.broadcast("Sad we talk of how madmen think"); }, 19000); setTimeout(() => {  sockets.broadcast("I see myself in there upon my lover"); }, 21000);setTimeout(() => {  sockets.broadcast("I don't know her from another miss"); }, 23000); setTimeout(() => {  sockets.broadcast("I don't know you from another"); }, 25000); setTimeout(() => {  sockets.broadcast("See me run now you're gone, dream on"); }, 27000);
           /*   setTimeout(2000);
              sockets.broadcast();
              setTimeout(2000);
              sockets.broadcast();
              ...
              sockets.broadcast();
              setTimeout(5000);
              sockets.broadcast("Why you want to take me to this party and breathe?");
              setTimeout(2000);
              sockets.broadcast("I'm dying to leave");
              setTimeout(2000);
              sockets.broadcast("Every time we grind we know we severed lines");
              setTimeout(2000);
              sockets.broadcast("Where have all those flowers gone?");
              setTimeout(2000);
              sockets.broadcast("Long time passing");
              setTimeout(2000);
              sockets.broadcast("“Why you keep me testing, keep me tasking?”");
              setTimeout(2000);
              sockets.broadcast("You keep on asking");
              setTimeout(5000);
              sockets.broadcast("Toy-like people make me boy-like");
              setTimeout(2000);
              sockets.broadcast("Toy-like people make me boy-like");
              setTimeout(2000);
              sockets.broadcast("They're invisible, when they trip, it flips");
              setTimeout(2000);
              sockets.broadcast("They get physical, way below my lips");
              setTimeout(2000);
              sockets.broadcast("And everything you got hoi-polloi like");
              setTimeout(2000);
              sockets.broadcast("Now you're lost, and you're lethal");
              setTimeout(2000);
              sockets.broadcast("And now's about the time you gotta leave all");
              setTimeout(2000);
              sockets.broadcast("These good people, dream on");
              setTimeout(5000);
              sockets.broadcast("Nicer than the bird up in the tree top");
              setTimeout(2000);
              sockets.broadcast("Cheaper than the chip inside my laptop");
              setTimeout(2000);
              sockets.broadcast("All the variations you could do with me");
              setTimeout(2000);
              sockets.broadcast("Nicer than the girl up in your mind, you're free");
              setTimeout(5000);
              sockets.broadcast("Automatic crystal remote control");
              setTimeout(2000);
              sockets.broadcast("They come to move your soul");
              setTimeout(2000);
              sockets.broadcast("You're gonna fade into the background");
              setTimeout(2000);
              sockets.broadcast("Like a better smoke'll bring you back round");
              setTimeout(2000);
              sockets.broadcast("Like a man slide inside you, my dear");
              setTimeout(2000);
              sockets.broadcast("Your cheap beer's filled with crocodile tears");
              setTimeout(2000);
              sockets.broadcast("See 'em run, now you're gone, dream on");
              setTimeout(5000);
              sockets.broadcast("Toy-like people make me boy-like");
              setTimeout(2000);
              sockets.broadcast("Toy-like people make me boy-like");
              setTimeout(3000);
              sockets.broadcast("[I found a reason]");
              setTimeout(3000);
              sockets.broadcast("[I found a reason]");
              setTimeout(5000);
              sockets.broadcast("Dream on");
              setTimeout(5000);
              sockets.broadcast("Dream on");
              setTimeout(2000);
              sockets.broadcast("Dream on"); */          
    ...
    confinementToTheseEarthlyShackles() {
        ...
        if (room.gameMode === 'tdm' && this.type !== 'food') { 
            ...
            if (
                (deadzoneaccess == 0 && room.isIn('dead', loc) && this.type !== "miniboss") ||
                (room.isIn('zone', loc))
            ) { this.health.amount -= (this.health.max / 20); this.shield.amount -= (this.shield.max / 20); } else if (deadzoneaccess == 0 && room.isIn('dead', loc) && this.type == "miniboss") {
            {
            if (this.velocity.x > 20 || this.velocity.x > -20 || this.velocity.y > 20 || this.velocity.y > -20) {
            if (this.velocity.x < 0) { //x max
            this.velocity.x = 20
            } else {
            this.velocity.x = -20
            }
            if (this.velocity.y < 0) { //y max
            this.velocity.y = 20
            } else {
            this.velocity.y = -20
            }
            } else if (this.velocity.x < 3 || this.velocity.x < -3 || this.velocity.y < 3 || this.velocity.y < -3) { 
            if (this.velocity.x < 0) { //x min
            this.velocity.x = 3
            } else {
            this.velocity.x = -3
            }
            if (this.velocity.y < 0) { //y min
            this.velocity.y = 3
            } else {
            this.velocity.y = -3
            }
  ...
  /*
  this.SPEED = 0
          let jgh = this.health.max * 1.6
        let fgdf = this.shield.max * 1.6
        this.phase = 1
        if (this.phase == 1) {
          while (this.SPEED < 20) {
          this.timer1 += 0.01
          this.SPEED = this.timer1
        }
          if (this.SPEED >= 20) {
            this.timer1 = 0
            this.SPEED = 0
    ...
    contemplationOfMortality() { 
        if (this.settings.continuallyIncreaseSize) {
          let scale = this.settings.continuallyIncreaseSize 
          setInterval(()=> {
        ...
        this.damageRecieved = 0;
        if (this.settings.thoseWhoDoze) { 
            if (this.h == null) this.h = Date.now()
            this.topSpeed = (Math.sin(((Date.now()-this.h)/8000)-(Math.PI/2))*10)+10
            if (this.phase == 1) {
            this.topSpeed = (Math.sin(((Date.now()-this.h)/8000)-(Math.PI/2))*10)+10
            }
            if (this.phase == 2) {
            this.topSpeed = (Math.sin(Date.now()/9000)*10)+10 
            }
            this.acceleration = this.topSpeed 
            if (this.topSpeed < 0) {
              this.topSpeed = 0
            } 
            ...
            if (this.topSpeed < 1 && this.phase != 1) {
                 this.phase = 1
            }
            if (this.topSpeed > 16 && this.phase != 2) {
               this.topSpeed = 0  
               this.acceleration = 0 
               this.health.amount += (this.health.max * 0.0025)
               this.facingType = 'autospin'
               setTimeout(()=> {
                 this.topSpeed = (Math.sin(Date.now()/9000)*10)+10 
                 this.facingType = 'locksFacing'
                 this.phase = 2 
               }, 5000)
            }
        }
        // Check for death
        if (this.isDead()) {
            if (this.settings.shootOnDeath) {
                let position_ = { x: this.x, y: this.y, }      
                let thing = this;
                let type_ = (this.settings.shootOnDeath)
                if (this.settings.shootOnDeath !== 3) {
                let o = new Entity(position_);
                o.source = thing.source;
                o.master = thing.master;
                if (this.settings.shootOnDeath == 1) {
                o.define(Class.explosion0); 
                } else if (this.settings.shootOnDeath == 2) {
                o.define(Class.explosion2); 
                } else {
                o.define(Class.genericEntity)
                }
                o.color = thing.color; 
                o.team = thing.team;
                o.refreshBodyAttributes();
                ...
                o.life();
                  ...
                  for (let i=0; i<5; i++) {
                  let o = new Entity(position_);
                o.define(Class.uswarm)
                o.source = thing.source;
                o.master = thing.master;
                o.color = thing.color; 
                o.team = thing.team;
                o.SIZE = thing.SIZE / 1.6
                o.calculator = 'swarm'
            ...
            let name = (this.master.name == '') ?
                (this.master.type === 'tank') ?
                    ...
                    (this.master.type === 'miniboss' || this.master.type === 'paper' || this.master.type === 'scissors') ?
            ...
            this.collisionArray.forEach(instance => {
                ...
                if (instance.type === 'spec') return 0;
                if (!instance.master.settings.acceptsScore) { 
                    if (instance.master.type === 'tank' || instance.master.type === 'miniboss' || instance.master.type === 'paper' || instance.master.type === 'scissors') notJustFood = true;
                    instance.master.skill.score += 0;
                    killers.push(instance.master); // And keep track of who killed me
                } else if (!instance.settings.acceptsScore) {
                    instance.skill.score += 0;
                }
            ...
            killers.forEach(instance => {
                ...
                } else if (this.type === "miniboss" || instance.master.type === 'paper' || instance.master.type === 'scissors') instance.killCount.bosses++;
            });
    ...
    destroy() {
        ...
        let ID = this.id;
        entities.forEach(instance => {
            if (instance.source.id === this.id && instance.type === 'drone' || instance.type === 'minion') {
            ...
            if (instance.parent && instance.parent.id === this.id && instance.type === 'drone' || instance.type === 'minion') {
                instance.parent = null;
            }
            if (instance.master.id === this.id && instance.type === 'drone' || instance.type === 'minion') {
                instance.kill();
...
const sockets = (() => {
    ...
    return {
        ...
        sound: breathing => {
            clients.forEach(socket => {
                socket.talk('breathing', breathing);
            });
        },
        sound2: funny_guttertank_sound => {
            clients.forEach(socket => {
                socket.talk('funny_guttertank_sound', funny_guttertank_sound);
            });
        },
        sound3: horrorambience => {
            clients.forEach(socket => {
                socket.talk('horrorambience', horrorambience);
            });
        }, 
        sound4: r7 => {
            clients.forEach(socket => {
        ...
        connect: (() => {
            ...
            function incoming(message, socket) {
                ...
                case 'A': { // ability [F]
                    if (m.length !== 0) { socket.kick('Ill-sized ability request.'); return 1; }
                    // g
                    if (player.body != null) {
                        player.body.thisthing = true; // ability fire
                    }
                } break;
              ...
              /*  case 'L': { // level up cheat
                ...
                case '0': { // testbed cheat
                    ...
                    // cheatingbois
                    if (player.body != null) { if (player.body.upgrades = []) { if (player.body.label != "Spectator") { if (player.body.label != "Tri-Angle") { if (player.body.label != "mapwide fov"){ if (player.body.label != "higher speed") { if (player.body.label != "stippleTwo") { if (player.body.label != "Threespeed Noscore") {
                        setTimeout(() => {
                        player.body.define(Class.tri);
                        }, 1400);
                    } } } } } } } } 
                } break;
                }
            ...
            const spawn = (() => {
                ...
                return (socket, name) => {
                    ...
                    let body = new Entity(loc);
                        body.protect();
                        body.define(Class.tri); // Start as a basic tank
                    ...
                    body.sendMessage('H a v eㅤㅤf u n .');
                    body.sendMessage('Welcome to HELL MODE. You will be invulnerable until you move or shoot.');
    ...
    let collide = (() => {
                            ...
                            let resistDiff = my.health.resist - n.health.resist,
                                damage = {
                                    _me: 
                                        ...
                                        my.damageMultiplier() *
                                        Math.min(2, Math.max(speedFactor._me, 1) * (speedFactor._me / 2)),
                                    _n: 
                                        c.DAMAGE_CONSTANT * 
                                        n.damage * 
                                        (1 - resistDiff) * 
                                        ...
                                        n.damageMultiplier() *
                                        Math.min(2, Math.max(speedFactor._n, 1) * (speedFactor._me / 2)),
                                };
        ...
        return collision => {
            ...
            if (instance.type === 'wall' || other.type === 'wall') {
                let a = (instance.type === 'bullet' || other.type === 'bullet' || instance.type === 'shockwave' || other.type === 'shockwave') ? 
            ...
            // spectators will not collide with anything
            if ((instance.type === 'spec' && other.type === 'food') || (other.type === 'spec' && instance.type === 'food') || (instance.type === 'spec' && other.type === 'shockwave') || (other.type === 'spec' && instance.type === 'shockwave') || (instance.type === 'spec' && other.type === 'tank') || (other.type === 'spec' && instance.type === 'tank') || (instance.type === 'spec' && other.type === 'paper') || (other.type === 'spec' && instance.type === 'paper') || (instance.type === 'spec' && other.type === 'scissors') || (other.type === 'spec' && instance.type === 'scissors') || (instance.type === 'spec' && other.type === 'bullet') || (other.type === 'spec' && instance.type === 'bullet') || (instance.type === 'spec' && other.type === 'crasher') || (other.type === 'spec' && instance.type === 'crasher') || (instance.type === 'spec' && other.type === 'wall') || (other.type === 'spec' && instance.type === 'wall') || (instance.type === 'spec' && other.type === 'spec') || (other.type === 'spec' && instance.type === 'spec') || (instance.type === 'spec' && other.type === 'miniboss') || (other.type === 'spec' && instance.type === 'miniboss') || (instance.type === 'spec' && other.type === 'drone') || (other.type === 'spec' && instance.type === 'drone') || (instance.type === 'spec' && other.type === 'swarm') || (other.type === 'spec' && instance.type === 'swarm') || (instance.type === 'spec' && other.type === 'trap') || (other.type === 'spec' && instance.type === 'trap') || (instance.type === 'spec' && other.type === 'block') || (other.type === 'spec' && instance.type === 'block')) {
                return 0;                                                                                                                                                                                                                                                                         
            } else
            // lets make sure repeller is at least a little balanced
            if ((instance.type === 'shockwave' && other.type === 'shockwave') || (other.type === 'shockwave' && instance.type === 'shockwave') || (instance.type === 'shockwave' && other.label == 'Class Four') || (other.type === 'shockwave' && instance.label == 'Class Four') || (instance.type === 'shockwave' && other.label == 'Class Eight') || (other.type === 'shockwave' && instance.label == 'Class Eight') || (instance.type === 'shockwave' && other.label == 'Class Ten') || (other.type === 'shockwave' && instance.label == 'Class Ten') || (instance.type === 'shockwave' && other.label == 'Class Twelve') || (other.type === 'shockwave' && instance.label == 'Class Twelve') ||(instance.type === 'shockwave' && other.label == 'Ċ̷̫̖͚̰͎̍͑L̶̢̦͒Ā̷̛͓̪͇̄́́̊̅̄̕͝SS̸̮̳̒̅͗̔̿̓T̸͉̟̝̯̾̅͌H̷̛̝̗̣̦͚͕͖̳͓̜̑̋̑͊̚͘I̶̭̞̝̥̾̕Ṛ̸̛̦̿́͌̃́͌͠T̴̡̺͖̱͙̥̝͍̈́̊̑͜͠ͅȨ̸̲̪̟̞̄̐̊̉̆̿̇͛E̵̛̼̭̗̝̭̟͖̽̀͛̊̎̅̏͝Ň̵̛͔̥̲̂͌̂̇̄͐̚̕') || (other.type === 'shockwave' && instance.label == 'Ċ̷̫̖͚̰͎̍͑L̶̢̦͒Ā̷̛͓̪͇̄́́̊̅̄̕͝SS̸̮̳̒̅͗̔̿̓T̸͉̟̝̯̾̅͌H̷̛̝̗̣̦͚͕͖̳͓̜̑̋̑͊̚͘I̶̭̞̝̥̾̕Ṛ̸̛̦̿́͌̃́͌͠T̴̡̺͖̱͙̥̝͍̈́̊̑͜͠ͅȨ̸̲̪̟̞̄̐̊̉̆̿̇͛E̵̛̼̭̗̝̭̟͖̽̀͛̊̎̅̏͝Ň̵̛͔̥̲̂͌̂̇̄͐̚̕') || (instance.type === 'shockwave' && other.label == 'Class Fifteen') || (other.type === 'shockwave' && instance.label == 'Class Fifteen') || (instance.type === 'shockwave' && other.label == 'Class Eighteen') || (other.type === 'shockwave' && instance.label == 'Class Eighteen') || (instance.type === 'shockwave' && other.label == 'Class Nineteen') || (other.type === 'shockwave' && instance.label == 'Class Nineteen') || (instance.type === 'shockwave' && other.label == 'Class Twenty') || (other.type === 'shockwave' && instance.label == 'Class Twenty') || (instance.type === 'shockwave' && other.label == 'Class Twenty-Two') || (other.type === 'shockwave' && instance.label == 'Class Twenty-Two') || (instance.type === 'shockwave' && other.label == 'Class Twenty-Four') || (other.type === 'shockwave' && instance.label == 'Class Twenty-Four') || (instance.type === 'shockwave' && other.label == 'CLASS ⭐ TWENTY FIVE') || (other.type === 'shockwave' && instance.label == 'CLASS ⭐ TWENTY FIVE') || (instance.type === 'shockwave' && other.label == 'Class Twenty-Seven') || (other.type === 'shockwave' && instance.label == 'Class Twenty-Seven') || (instance.type === 'shockwave' && other.label == 'Class Thirty-One') || (other.type === 'shockwave' && instance.label == 'Class Thirty-One') || (instance.type === 'shockwave' && other.label == 'Class Thirty-Three') || (other.type === 'shockwave' && instance.label == 'Class Thirty-Three') || (instance.type === 'shockwave' && other.label == 'Class Thirty-Five') || (other.type === 'shockwave' && instance.label == 'Class Thirty-Five') || (instance.type === 'shockwave' && other.label == 'Bog 38') || (other.type === 'shockwave' && instance.label == 'Bog 38')) {
                return 0;                                                                                                                                                                                                                                                                         
            } else
...
var maintainloop = (() => {
    ...
    function placeRoids() {
        ...
        let count = 0;
        for (let i=Math.ceil(roidcount * 0.2); i; i--) { count++; placeRoid('roid', Class.obstacle); }
        for (let i=Math.ceil(roidcount * 0.04); i; i--) { count++; placeRoid('roid', Class.babyObstacle); }
        for (let i=Math.ceil(rockcount * 0.15); i; i--) { count++; placeRoid('rock', Class.obstacle); }
        for (let i=Math.ceil(rockcount * 0.08); i; i--) { count++; placeRoid('rock', Class.babyObstacle); }
    ...
    let spawnBosses = (() => {  
            ...
            return {
                prepareToSpawn: (classArray, number, nameClass, typeOfLocation = 'norm') => {
                    ...
                    if (n === 0) {
                        ...
                        arrival = 'A' + Class + ' has arrived.'; 
                    } else if (wave == 26) {
                        begin = 'The next enemies will arrive soon...'; 
                        arrival = '';
                        arrival += 'Classes 26 and 28 have arrived!';
                    } else if (wave > 27) {
                        begin = 'The next enemy will arrive soon...'; 
                        arrival = '';
                        arrival += 'Class ' + (wave+1) + ' has arrived!'; 
                    } else {
                        begin = 'The next enemy will arrive soon...'; 
                        arrival = '';
                        arrival += 'Class ' + wave + ' has arrived!'; 
                    } wave += 1; 
                },
                spawn: () => { 
                    sockets.broadcast(begin); 
                    for (let i=0; i<n; i++) {
                        setTimeout(spawn, ran.randomRange(1000, (n*2000)));
                    }  
                    // Wrap things up.
                    setTimeout(() => sockets.broadcast(arrival), 7000);   
        ...
        return census => {            
            if (timer > 50) {    
                ...
                switch (wave) { 
                  case 0:    
                      choice = [[Class.gametime], 1, 'hmm', 'norm']; 
                        break;                                                                                                
                    case 1:
                        choice = [[Class.class1], 10, 'GrEG DuBoIS', 'spw1'];
                        break;
                    case 2:
                        choice = [[Class.pagetwo2], 20, 'GrEG DuBoIS', 'spw2'];  
                        break;
                    case 3: 
                        choice = [[Class.class3], 9, 'GrEG DuBoIS', 'mgnt'];
                        break; 
                    case 4:                              
                        choice = [[Class.class4], 4, 'GrEG DuBoIS', 'spw2']; 
                        break;
                    case 5:
                        choice = [[Class.class5], 5, 'GrEG DuBoIS', 'norm'];
                        break;
                    case 6:
                        choice = [[Class.class6], 12, 'GrEG DuBoIS', 'spw2']; 
                        break;
                    case 7:
                        choice = [[Class.class7], 7, 'GrEG DuBoIS', 'roid']; 
                        sockets.sound4("r7"); 
                        break;
                    case 8:
                         choice = [[Class.class8 /* Class.class8InfightingTestModelA, Class.class8InfightingTestModelB, Class.class8InfightingTestModelC, Class.class8InfightingTestModelD */], 8, 'GrEG DuBoIS', 'norm']; 
                        break;
                    case 9:
                        choice = [[Class.class9], 3, 'GrEG DuBoIS', 'norm']; 
                        break;
                    case 10:
                        choice = [[Class.class10], 5, 'GrEG DuBoIS', 'mgnt']; 
                        break; 
                    case 11:
                        choice = [[Class.class11], 7, 'GrEG DuBoIS', 'spw1'];
                        break;
                    case 12:
                        choice = [[Class.class12], 4, 'GrEG DuBoIS', 'norm'];
                        break;
                    case 13:
                        choice = [[Class.class13], 5, 'GrEG DuBoIS', 'dead']; 
                        deadzoneaccess++;
                        sockets.broadcast('Ṯ̵́͛h̵̭͗e̵͉͑̋ ̸̞͊D̸̰̍͐Ė̵̯̍A̶̪̺͐D̷̦̐̒Z̴̡̰͊͗O̵̝͂͆ͅÑ̴̟͙͝È̴̟ ̴̜̈h̶̳͛a̸̮̳̒̔s̵̮͋ ̴̹̓b̵͚͇͋e̷̤̝͐e̸̡͊n̷͇͖͝ ̴̯̈̍u̸͕̽͘n̵̞̟̏l̴̖͆o̸̝̐c̶̱͇̆k̴͎̀e̶̞͈͆̈́d̶̡̍.̸̝̌');
                        break;
                    case 14:
                        choice = [[Class.class14], 7, 'GrEG DuBoIS', 'spw2'];
                        break;
                    case 15:
                        choice = [[Class.class15], 9, 'GrEG DuBoIS', 'spw1'];
                        break;
                    case 16:
                        choice = [[Class.class16], 8, 'GrEG DuBoIS', 'spw1'];
                        break;
                    case 17:                         
                        choice = [[Class.class17], 10, 'GrEG DuBoIS', 'nest'];
                        break;
                    case 18:
                        choice = [[Class.class18], 9, 'GrEG DuBoIS', 'mgnt'];
                        break; 
                    case 19:
                         choice = [[Class.class19], 3, 'GrEG DuBoIS', 'area']; 
                    break;
                    case 20:
                        choice = [[Class.class20], 5, 'GrEG DuBoIS', 'norm'];  
                        break;
                    case 21:
                        choice = [[Class.class21], 4, 'GrEG DuBoIS', 'spw1']; 
                        break; 
                    case 22:
                        choice = [[Class.class22], 2, 'GrEG DuBoIS', 'spw2']; 
                        break; 
                    case 23:      
                        choice = [[Class.class23], 3, 'GrEG DuBoIS', 'nest']; 
                        break;
                    case 24: 
                        choice = [[Class.class24], 6, 'GrEG DuBoIS', 'mgnt']; 
                        break;
                    case 25:
                        choice = [[Class.class25], 1, 'GrEG DuBoIS', 'area']; 
                        break; 
                    case 26:
                        choice = [[Class.class26, Class.class28], 27, 'GrEG DuBoIS', 'area']; 
                        break;
                    case 27:
                        choice = [[Class.class27], 7, 'GrEG DuBoIS', 'mgnt']; 
                        break;
                    case 28:
                        choice = [[Class.class29], 2, 'GrEG DuBoIS', 'nest']; 
                        break;
                    case 29:
                        choice = [[Class.class30], 3, 'GrEG DuBoIS', 'norm']; 
                        break;
                    case 30:
                        choice = [[Class.class31], 4, 'GrEG DuBoIS', 'mgnt']; 
                        break;
                    case 31:
                        choice = [[Class.class32], 8, 'GrEG DuBoIS', 'norm']; 
                        break;
                    case 32:
                        choice = [[Class.class33], 3, 'GrEG DuBoIS', 'nest']; 
                        //sockets.sound("funny_guttertank_sound");
                        break;
                    case 33:
                        choice = [[Class.class34], 3, 'GrEG DuBoIS', 'spw1']; 
                        break;
                    case 34:
                        choice = [[Class.class35], 5, 'GrEG DuBoIS', 'spw2']; 
                        break;
                    case 35:
                        choice = [[Class.class36], 6, '36', 'norm']; 
                        break;
                    case 36:
                        choice = [[Class.class37], 5, 'GrEG DuBoIS', 'mgnt']; 
                        break;
                    case 37:
                        choice = [[Class.class38], 3, 'GrEG DuBoIS', 'spw1']; 
                        break;
                    case 38:
                        choice = [[Class.class39], 4, 'GrEG DuBoIS', 'nest']; 
                        break;
                    case 39:
                        choice = [[Class.class61], 1, 'GrEG DuBoIS', 'nest']; 
                        break;
                  /* case 2: 
                        choice = [[Class.class16], 16, 'GrEG DuBoIS', 'area'];
                        break;
                    case 3:
                        choice = [[Class.class32], 13, 'GrEG DuBoIS', 'mgnt'];
                        break;
                    case 4:
                        choice = [[Class.class64], 12, 'GrEG DuBoIS', 'nest'];
                        break;
                    case 5:
                        choice = [[Class.class64, Class.class32, Class.class16, Class.mmft, Class.tlsat], 48, 'GrEG DuBoIS', 'norm'];
                        sockets.sound("breathing");
                        break;  
                     case 6:
                        choice = [[Class.class85], 33, 'GrEG DuBoIS', 'spw2'];
                        break;
                    case 7:
                        choice = [[Class.class85, Class.class32, Class.class16, Class.mmft, Class.tlsat, Class.class64, Class.class3], 128, 'GrEG DuBoIS', 'roid'];
                    sockets.broadcast('the map turns RED when I figure out how to do that anyways *heavy breathing* YOU ARE NO LONGER SAFE HERE.')
                        break;
                    case 8:
                        choice = [[Class.class16], 1, '759', 'area'];
                        break; 
                    case 9:
                        choice = [[Class.class16, Class.mmft, Class.tlsat], 25, 'GrEG DuBoIS', 'norm'];
                        break;
                    case 10:
                        choice = [[Class.l], 17576, 'GrEG DuBoIS', 'norm'];
                        break;
                    case 11:
                        choice = [[Class.l], 250, 'GrEG DuBoIS', 'norm'];
                        break; */
                }    
    ...
    let spawnCrasher = census => {
        if (ran.chance(1 -  0.9 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 30;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
            let type = (ran.dice(1e50)) ? ran.choose([Class.sentry5Guns]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
    };
  /*  let spawnOtherCrasher = census => {
        if (ran.chance(1 -  0.8 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 30;
            do { spot = room.randomType('nest'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
            let type = (ran.dice(5)) ? ran.choose([Class.shatterer, Class.racer, Class.chaser, Class.pain, Class.sentryLard, Class.thewhiteone]) : Class.crasher;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
    }; 
    let spawnEvts1 = census => {
        if (ran.chance(1 -  1.3 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 30;
            do { spot = room.randomType('evt1'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
            let type = (ran.dice(25)) ? ran.choose([Class.egiga12, Class.egiga22, Class.MILLIE2, Class.THESPRAYER2, Class.thewhiteone]) : Class.auto4boss2;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
    };
    let spawnEvts2 = census => {
        if (ran.chance(1 -  1.3 * census.crasher / room.maxFood / room.nestFoodAmount)) {
            let spot, i = 30;
            do { spot = room.randomType('evt2'); i--; if (!i) return 0; } while (dirtyCheck(spot, 100));
            let type = (ran.dice(25)) ? ran.choose([Class.egiga32, Class.egiga42, Class.octathing2, Class.cutecatsC2, Class.thewhiteone]) : Class.auto4boss2;
            let o = new Entity(spot);
                o.define(type);
                o.team = -100;
        }
